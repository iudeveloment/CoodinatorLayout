...
* **AppBarLayout**  
  Toolbar, 기타 View가 ScrollingViewBehavior로 표시된 형제 View에서 일어난 Scroll Event로 반응하도록 설계

* **CollapsingToolbarLayout**  
  Text Size, pinning and parallax  
  확장된 높이의 Toolbar를 접으면 Title Size 조정 가능.  
  접을 수 있는 Toobar 위로 항목을 고정시키거나, 변화하는 이미지를 넣거나, 접히면 원색을 화면에 표현 가능.
...
RecyclerView를 Scroll 하면 **AppBarLayout이 Touch Event에 응답!**  
...
## Behavior
**CoordinatorLayout의 Child View에 대한 Behavior 지정으로 단일 Parent, View 간 다양한 상호 작용 가능.**
...
#### NestedScrollView
NestedScrollView의 layout_behavior는 **AppBarLayout$ScrollingViewBehavior** 가 미리 정의됨.      
NestedScrollView의 반응에 따라 AppBarLayout이 반응.  

CoordinatorLayout는 NestedScrollView가 Scroll 되면 layout_behavior로 정의된 레이아웃으로 Scroll 정보를 전달 하는 역할. 그럼 AppBarLayout의 ScrollingViewBehavior 정보로 AppBarLayout 자신을 변형하도록 하는 구조.
...
## Anchor
**특정 View의 Anchor를 달아 고정하고 함께 이동함.**  
Anchor를 고정하는 위치 및 기준 지정 가능.
